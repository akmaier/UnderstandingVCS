...
**Totals after P4d + PXC1-x round 3 + P7e-x extension: jaxtari 550 tests (+ 1 xfail), jutari 1136 tests (+ 1 broken) — 1687 effective.** One xfail remains, recording the PXC1-x round 2+ bit-exact gap (10 RAM bytes).
...
- **PXC1-x round 3 is ✅ shipped (jaxtari + jutari)** — the real-hardware TIA **floating-bus quirk** now lands on both ports. The 1A05 TIA only actively drives 1-2 of the 8 data lines on a read; the rest "float" and resolve to whatever was last on the bus. xitari emulates this in `System::peek/poke` (which update `myDataBusState`) + `TIA::peek` (which OR's the noise into every collision/INPT/unused-register return). jaxtari's `Bus` NamedTuple gained a `data_bus_state` field updated on every peek/poke; `_bus_peek` masks the TIA-driven bits and OR's noise into the rest using per-register driven-bit masks ($C0 for collision-pairs, $80 for CXBLPF/INPT, $00 for $0E/$0F). jutari mirrors the same on its mutable `BusState`. Tests: 12 new (6 per port). **Conformance status**: the `pong_noop_10` 10-byte divergence pattern is **unchanged** by this fix — the bytes that differ come from RAM-to-RAM ops in Pong's frame logic, not direct TIA reads, so the underlying wrong-input chains back further. The floating-bus fix is real-hardware-correct (real games whose stored values aren't masked to D7/D6 before use will benefit) — just not the proximate cause of this fixture's residual gap, which now needs a **per-cycle CPU/TIA trace harness** as its next diagnostic.
