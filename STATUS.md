...
- **P6c is ✅ partial — Pong + Breakout + Space Invaders landed** — `PongRomSettings` (ΔP0 − ΔP1, terminal at 21), `BreakoutRomSettings` (Δscore from BCD-packed $76/$77, terminal at lives = 0, exposes lives count), `SpaceInvadersRomSettings` (ΔBCD score from $E8/$E9, terminal at lives = 0). All three handle RAM addresses in the $80-$FF mirror correctly by masking with 0x7F. Tests: 13 in `test_p6c_pong.py` + 14 in `test_p6c_more_games.py` covering reward deltas, terminal at lives = 0 (or score = target), score decoding, reset clearing previous-score state, constants, RomSettings protocol conformance. Pitfall / Asteroids / Q*Bert etc. follow the same pattern (each game's RAM probes live in `xitari/src/games/supported/<Game>.cpp`).
