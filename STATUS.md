...
- **P1h is ✅ partial — common undocumented opcodes landed (jaxtari HARD)** — 18 NOP variants (1-byte implied $1A/$3A/$5A/$7A/$DA/$FA; 2-byte imm/zp/zp,X; 3-byte abs/abs,X with page-cross penalty), 6 LAX modes (load A and X from operand, set N/Z), 4 SAX modes (store A AND X, no flag effects). The "magic AND" LAX #imm ($AB) and the RMW combos (DCP / ISC / RLA / RRA / SLO / SRE) stay deferred — they're rare and $AB is unstable on real hardware. Tests: `jaxtari/tests/test_p1h_undocumented.py` (21). Jutari mirror and SOFT-mode mirror are the obvious extensions; the HARD path is where real-ROM correctness lives.

### Bus (P2)
- Settled: 6507 13-bit mirror, RAM with stack-page mirror. **No deferrals.**

### TIA (P3)
- **P3g is ✅ partial — jaxtari (HARD + SOFT)** — all 8 NUSIZ low-3-bit layouts honoured: 1/2/3 copies with close (16-pixel) / medium (32-pixel) / wide (64-pixel) spacing, plus 2× and 4× single-copy scaling. Missiles inherit the same multi-copy from NUSIZ (their per-copy *width* still comes from NUSIZ bits 4-5). Collision-set computation respects the new layout so multi-copy hits register. SOFT renderer blends all 8 modes via a one-hot dot product on the integer NUSIZ — gradient through every colour register still exact; gradient through NUSIZ itself is the same structural-bit story as CTRLPF.D2. Tests: `jaxtari/tests/test_p3g_nusiz.py` (16) — every HARD mode pixel-by-pixel, 2× pattern check, missile inheritance, SOFT gradient reaching the second copy, NUSIZ=0 default-mode regression guard. **Jutari mirror is the obvious follow-up.**
...
### Cart (P5)
- **P5b is ✅ partial — F8SC landed** — F8SC cart format (F8 + 128 B on-cart RAM at $1000–$10FF) handled by `cart_peek` / `cart_poke`: writes to $1000–$107F land in the cart's 128-byte buffer, reads from $1080–$10FF mirror them back, bank switching otherwise identical to F8. Selectable via `make_cart(rom, kind=KIND_F8SC)` / `initial_bus(rom, cart_kind=KIND_F8SC)` — size alone can't disambiguate F8 from F8SC. Tests: 17 in `jaxtari/tests/test_p5b_f8sc.py` covering construction, RAM round-trip, mirror correctness, bank-switching coexistence, and bus-level peek/poke. F6SC + F4SC are the obvious extensions (same shape, more banks).
...
- **P6c is ✅ partial — Pong landed** — `PongRomSettings` reads the canonical P0/P1 score bytes from RAM[$14]/RAM[$15], reports per-step reward as ΔP0 − ΔP1, and terminates when either side reaches 21 points. Available as `from jaxtari.games import PongRomSettings`; pass `StellaEnvironment(rom, PongRomSettings())` to drive a real Pong agent. Tests: 13 in `jaxtari/tests/test_p6c_pong.py`. Breakout / Space Invaders / Pitfall / Asteroids etc. are the obvious extensions (each game's RAM probes live in `xitari/src/games/supported/<Game>.cpp` as a reference).
...
- **Still deferred after P7f** (each a deliberate, documented gap, not a bug):
  - **P7f-dx is ✅ complete** — TIA collision reads now dispatch through `_bus_read`: addresses `$30-$37` route to `soft_collision_registers(bus)[addr & 0x07]` via a one-hot dot product instead of falling through the SOFT RAM collapse. A SOFT program doing `LDA $30` (CXM0P) now gets the live collision latch. The selection is a `jnp.where` chain (cart / RIOT-I/O / collision / RAM) so the gradient flows through whichever branch is selected. Tests: 9 in `jaxtari/tests/test_p7f_dx.py` — direct `_bus_read($30..$37)` returns the right latch for every overlap case, end-to-end SOFT program loads a collision into A, non-collision TIA reads stay on the RAM-collapse path, collision read gradient is zero (structural threshold).
