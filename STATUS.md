...
- **PXC1-x round 2+** (stalled, needs new tooling): the remaining 10 bytes are deep TIA scanline-cycle / RIOT timing detail. Diagnosing further frame-level RAM-only — which is all xitari's public ALEInterface exposes — has hit its information ceiling. Closing them needs a per-cycle xitari debug-trace patch (PORTING_PLAN.md §4.1 sketches it: roughly 100–300 LOC of C++ extending `Console`, `System`, `M6502`, `TIA`, `M6532` to dump CPU + TIA + RIOT state). That patch is its own substantial work item and is the prerequisite for further PXC1-x rounds.
