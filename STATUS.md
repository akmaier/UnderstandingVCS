...
- **P6c is ✅ partial — six game scorers landed** — `PongRomSettings` (ΔP0 − ΔP1, terminal at 21), `BreakoutRomSettings` (Δscore from BCD-packed $76/$77, terminal at lives = 0), `SpaceInvadersRomSettings` (ΔBCD score from $E8/$E9, terminal at lives = 0), and three more in `jaxtari/games/more_games.py`: `AsteroidsRomSettings` (Δscore from $3E/$3D × 10 with 100000-score wrap, lives from $3C high nibble), `QbertRomSettings` (3-byte BCD score from $DB/$DA/$D9, terminal sentinel = $FE in $88, no-reward-on-terminal-frame), `MsPacmanRomSettings` (3-byte BCD score from $F8/$F9/$FA, terminal at lives = 0 AND $A7 = $53). All six handle RAM addresses in the $80-$FF mirror correctly via `addr & 0x7F`. Tests: 13 in `test_p6c_pong.py` + 14 in `test_p6c_more_games.py` + 15 in `test_p6c_arcade_games.py`. Pitfall / Beamrider / Enduro / Seaquest remain — each follows the same pattern (RAM probes in `xitari/games/supported/<Game>.cpp`).
