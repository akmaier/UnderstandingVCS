...
| **P7c** | Extend SOFT-mode opcode coverage to the full 151-opcode documented NMOS set; add N/Z/C/V flag updates inside handlers. Subdivided **P7c-a … P7c-f** matching P1's subdivision. | All 151 documented NMOS opcodes (+ USBC `$EB`) execute in SOFT mode | ✅ — jaxtari `soft_step.py` + jutari `SoftStep.jl`; 152-opcode dispatch table; jaxtari 449 tests / jutari 1022 tests. Three deliberate simplifications remain (P7c-bx BCD, P7c-dx branch-predicate gradient, BRK-as-sentinel). |
...
- **P7c-b**: arithmetic/logic (ADC/SBC/AND/ORA/EOR/CMP/CPX/CPY/BIT) with N/Z/C/V flag updates. **BCD (decimal-mode) ADC/SBC is NOT implemented** — the binary path always runs regardless of the D flag (Atari ROMs almost never set decimal mode; the 2A03 ignored it). Real BCD support is deferred as **P7c-bx**.
