...
| **P7f** | Differentiable TIA so `jax.grad` flows from a framebuffer pixel back to ROM bytes. Subdivided **P7f-a … P7f-d** mirroring P3. | `jax.grad` of a framebuffer pixel is one-hot at the ROM byte that painted it | ✅ — **P7f-a/b/c/d** render the full visible object set (background, playfield, players, missiles, ball) + 8 collision latches; `∂pixel/∂ROM` works end-to-end. Deferred: VBLANK blanking, collision *reads* (P7f-dx), cart-hotspot bank-switching (P7f-e). |
| **P8** | XAI hooks + first attribution experiment. Subdivided **P8-a/b/c**. | Integrated Gradients on ROM bytes recovers a known sprite-defining region | ✅ — **P8-a** IG primitive (7 unit tests); **P8-b** three attribution semantics on a SOFT kernel (4 tests); **P8-c** Pong-like state attribution (6 tests + 1 xfail) plus `soft_run_scan` (jax.lax.scan-backed) for thousands-of-instruction traces. Real-Pong-execution attribution is **P8-cx**, blocked on SOFT-mode RIOT timer (Pong's startup busy-waits on INTIM). |
| **P9** | JAX-vs-Julia benchmark + first paper-shaped XAI study | Throughput numbers + a writeup | ✅ — `tools/bench_soft_run.{py,jl}` benchmarks SOFT throughput per instruction; `RESULTS.md` is the writeup. Headline: jutari ~218× faster than jaxtari on SOFT throughput (JAX kernel-launch overhead at single-opcode granularity). |
...
- **P7f-b** ✅: player sprites P0 / P1 — GRP graphics, REFP reflect, single 1×-wide copy (NUSIZ multi-copy deferred, matching the HARD P3c deferral). Composite over the playfield with TIA priority (P0 on top). SOFT-mode convention: the `RESP0`/`RESP1` cells hold the player X position (faithful strobe-timing positioning waits for P7f-d).
- **P7f-c** ✅: missiles M0 / M1 + ball BL — solid 1/2/4/8-pixel blocks (`_block_mask`); missile size from NUSIZ bits 4-5, ball size from CTRLPF bits 4-5, enabled by ENAM*/ENABL bit 1. Composited in the full HARD priority order: background ← playfield ← ball ← M1 ← P1 ← M0 ← P0.
- **P7f-d** ✅: collision latches — `soft_collision_registers` detects the 15 pairwise object overlaps and packs them into the 8 CX registers (CXM0P…CXPPMM), matching the HARD TIA P3e layout. Collisions are a structural boolean property — forward feature parity, not a gradient path.
- **P7f-dx** (deferred): wire `soft_collision_registers` into `_bus_read` so a program can *read* $00-$07 as collision data — the "proper TIA read-register dispatch" piece. The write side already works: the SOFT bus's "TIA registers = `ram[0:64]`" collapse means a `STA $09` and the renderer's COLUBK read hit the same cell, which is consistent — so the originally-planned full register-dispatch rework turned out unnecessary for rendering.
- **P7f-e** (deferred): cart-hotspot bank-switching from SOFT mode — needs the SoftBus to carry bank state, an architectural addition.

> **Architectural prerequisite for P7f-b…d.** P7f-a works because background + playfield are pure functions of *static* register values, which the SOFT bus already holds in `ram[0:64]`. Sprites, missiles and the ball are not: their horizontal positions are set by **strobe** writes (`RESP0`/`RESP1`/`RESM*`/`RESBL`) whose effect depends on the beam position *at the cycle of the write* — dynamic timing state the static-register-file model does not capture. P7f-b…d therefore first need the `SoftBus` to carry real TIA timing state and `soft_step` to advance it (the differentiable counterpart of `tia_advance`). That is a genuine architectural increment, not "more of P7f-a", and should be planned as such before P7f-b starts.
...
- **P4c**: Paddle dump-pot timing — INPT0-3 stay at `$80` "centred" (PXC1-x round 4 diagnostic confirms paddle reads are the chained source of the `pong_noop_10` divergence). **Round 4.5 + PXC2 multi-ROM finding** (4 ROMs sampled): the per-ROM divergence count is **proportional to how many bus-noise-affected reads each ROM does during boot**, not a single switch on "is paddle game". Concretely — pong: 10 bytes (paddle reads INPT0-3); breakout: 4 (paddle, fewer cells touched); space invaders: 0 (joystick, minimal init reads); pitfall: 19 (joystick, but reads INPT5 trigger + RIOT timer more aggressively). So the proximate cause is **the same** across all 4 (data_bus_state drift from missing 6502 internal-cycle bus exposures — see PXC1-x round 4.5 + per-cycle bus accuracy deferral), but the manifestation is ROM-specific. The full fix is still **per-cycle 6502 bus accuracy** (RMW dummy-writes, JSR pre-push discard, branch extra-cycle, indirect dummy reads); the dump-pot model is a useful refinement on top of that for actual paddle gameplay.
