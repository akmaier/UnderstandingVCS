...
| **P8** | XAI hooks + first attribution experiment. Subdivided **P8-a/b/c**. | Integrated Gradients on ROM bytes recovers a known sprite-defining region (initially in a hand-written kernel; in Pong once PXC1-x lands) | 🟡 in progress — **P8-a ✅** (IG primitive: midpoint Riemann, completeness axiom, 7 unit tests). **P8-b ✅** (three attribution semantics on a SOFT kernel — plain gradient, occlusion, smart-baseline IG — plus the recorded finding that naive zero-baseline IG collapses on opcode bytes). **P8-c** waits for PXC1-x before real-Pong attribution is meaningful. |
