...
attribution methods across the 42 scored games, with the six core games (Pong, Breakout,
...
position regime the gradient and correlational family reaches only $0.23$ against the
...
causal and intervention family $0.72$. The position map is not empty by choice; it is
...
all-regime faithfulness of $0.18$ is the lowest among the attribution methods.
...
of $0.687$ is the highest among the attribution methods. The black-box perturb-and-re-run
methods follow the same logic and result:
KernelSHAP / Shapley sampling~\cite{lundberg2017unified,strumbelj2014explaining}
($0.652$), LIME~\cite{ribeiro2016why} ($0.644$), and RISE~\cite{petsiuk2018rise}
($0.552$) each draw masked coalitions and average over the genuine responses, so each
...
faithfulness $0.35$ and $0.28$, all-regime $0.46$ and $0.43$). The
counterfactual is a no-op on some games, where no on-distribution content byte varies
...
intervention family averages $0.72$ faithfulness [$n=11$]
...
widely used baseline, reaches only $0.115$. This is a per-method position gap of $0.885$.
...
(\Sectionname\ref{sec:oracle}).} Twelve attribution methods aggregated over the 42 scored games
...
leads ($0.72$ against $0.38$, position $0.56$ against $0.23$), and the popular gradient
...
\midrule
Occlusion~\cite{zeiler2014visualizing}            & interv.\  & $0.69$ $[0.62,0.75]$ & $0.62$ & $0.55$ \\
KernelSHAP~\cite{lundberg2017unified}             & black-box & $0.65$ $[0.59,0.72]$ & $0.60$ & $0.90$ \\
LIME~\cite{ribeiro2016why}                        & black-box & $0.64$ $[0.58,0.71]$ & $0.59$ & $0.90$ \\
RISE~\cite{petsiuk2018rise}                        & black-box & $0.55$ $[0.49,0.62]$ & $0.44$ & $0.90$ \\
Grad$\times$Input / DeepLIFT~\cite{shrikumar2017learning} & gradient & $0.49$ $[0.40,0.58]$ & $0.12$ & $0.90$ \\
Extremal perturbation~\cite{fong2019understanding} & interv.\  & $0.46$ $[0.40,0.52]$ & $0.35$ & $0.55$ \\
On-distribution counterfactual~\cite{olson2021counterfactual} & interv.\  & $0.43$ $[0.35,0.51]$ & $0.28$ & $0.55$ \\
SmoothGrad~\cite{smilkov2017smoothgrad}           & gradient & $0.38$ $[0.29,0.47]$ & $0.12$ & $0.90$ \\
Vanilla saliency~\cite{simonyan2014deep}          & gradient & $0.38$ $[0.29,0.47]$ & $0.12$ & $0.90$ \\
Integrated Gradients~\cite{sundararajan2017axiomatic} & gradient & $0.38$ $[0.29,0.46]$ & $0.11$ & $0.90$ \\
Guided backprop~\cite{springenberg2015striving}   & gradient & $0.32$ $[0.24,0.41]$ & $0.00$ & $0.90$ \\
Expected Gradients~\cite{erion2021improving}      & gradient & $0.18$ $[0.11,0.25]$ & $0.03$ & $0.90$ \\
\midrule
\multicolumn{2}{@{}l}{Gradient/corr.\ mean}  & $0.38$ & $0.23$ & --- \\
\multicolumn{2}{@{}l}{Causal/interv.\ mean}  & $0.72$ & $0.56$ & --- \\
\botrule
\end{tabularx}
\end{table}
