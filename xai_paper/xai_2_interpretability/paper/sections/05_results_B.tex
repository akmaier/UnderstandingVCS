...
gradient (Paper~1, and Sec.~\ref{sec:methods}). Hence the whole gradient family hits this
...
\emph{content} output the gradient family identifies the true cause, but on a
\emph{position}/index output the entire gradient family \emph{collapses to $0.00$}.
The intervention methods still work, because each probe is a real re-run of the
bit-exact program. Note that the popular gradient methods, which fail on position,
have the higher plausibility proxy, the danger zone of Sec.~\ref{sec:results_compare}
...
The gradient family fails on the position output. Its attribution goes to zero. The naive
...
alone, $0.068$. Returning an empty map
for half of a game's outputs is not partial success. It is a structural failure, exactly
where the discrete game logic is.
...
When we replace the gradient with a real intervention, the position failure
disappears. Occlusion~\cite{zeiler2014visualizing} sets a candidate cause and re-runs
the bit-exact program, a coarse version of the oracle. On the Pong position output,
...
$0.298$. A perturbation is faithful only when it is a valid intervention.
...
concrete. On the position regime the causal method activation patching (Phase~C) reaches
faithfulness $1.000$, the oracle ceiling, on all six games, while vanilla saliency, a
widely used baseline, collapses to $0.000$ on all six. This is a per-method gap of
$1.000$, bit-exact on every game. Yet, on the plausibility proxy of Sec.~\ref{sec:methods}
the popular method has the higher score ($0.9$ versus $0.5$): the empty map has a higher
plausibility score than the exactly correct one. Low faithfulness with high plausibility
...
Sec.~\ref{sec:results_compare}. Here we have established only that faithfulness is
