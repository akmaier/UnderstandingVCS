...
Every attribution is scored against the same intervention oracle (\S\ref{sec:oracle})
...
(Phase~C) scored against the same intervention oracle (\S\ref{sec:oracle}).} The
...
6502 CPU with a TIA and a RIOT executing a fixed ROM, so across all six games the
