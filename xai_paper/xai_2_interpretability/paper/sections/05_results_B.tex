...
intervention oracle in Table~\ref{tab:phaseB}.
...
the cross-tradition gradient family scores only $0.294$ faithfulness against the oracle
(Table~\ref{tab:phaseB}; the attribution gradient methods alone $0.298$); on the position regime
...
correlation $0.837$, its all-regime faithfulness of $0.482$ the highest among the attribution methods. The black-box perturb-and-re-run methods follow the same logic and result:
...
cross-tradition intervention family averages $0.681$ faithfulness [95\% CI $0.59$--$0.78$,
$n=11$] against the oracle, more than double the cross-tradition gradient family's $0.294$
[$0.20$--$0.38$, $n=14$] (Table~\ref{tab:phaseB}). Restricted to the three attribution
intervention methods alone the mean is $0.393$ against the attribution gradient family's
...
measurement. The entire gradient family scores $0.00$ on position while the intervention
family does not, and the popular gradient methods carry the \emph{higher} plausibility tag
--- the danger zone, measured. The two family-mean rows are the cross-tradition families
($n=14$ gradient/correlational, $n=11$ causal/intervention across all three traditions),
not the twelve rows above; restricted to attribution alone the gradient and intervention
means are $0.298$ and $0.393$. Grad-CAM, Grad-CAM++, attention rollout, and
...
\begin{tabular}{@{}p{3.1cm}llcc@{}}
\toprule
Method & Family & $F$ ($95\%$ CI) & $F_{\text{pos}}$ & Plaus \\
\midrule
Occlusion~\cite{zeiler2014visualizing}            & interv.\  & $0.48$ $[0.26,0.68]$ & $0.31$ & $0.55$ \\
KernelSHAP~\cite{lundberg2017unified}             & black-box & $0.45$ $[0.26,0.62]$ & $0.28$ & $0.90$ \\
LIME~\cite{ribeiro2016why}                        & black-box & $0.38$ $[0.19,0.57]$ & $0.17$ & $0.90$ \\
RISE~\cite{petsiuk2018rise}                        & black-box & $0.37$ $[0.19,0.56]$ & $0.17$ & $0.90$ \\
Grad$\times$Input / DeepLIFT~\cite{shrikumar2017learning} & gradient & $0.37$ $[0.26,0.45]$ & $0.00$ & $0.90$ \\
On-distribution counterfactual~\cite{olson2021counterfactual} & interv.\  & $0.35$ $[0.16,0.54]$ & $0.14$ & $0.55$ \\
Extremal perturbation~\cite{fong2019understanding} & interv.\  & $0.35$ $[0.18,0.50]$ & $0.20$ & $0.55$ \\
Integrated Gradients~\cite{sundararajan2017axiomatic} & gradient & $0.28$ $[0.14,0.41]$ & $0.00$ & $0.90$ \\
Guided backprop~\cite{springenberg2015striving}   & gradient & $0.27$ $[0.12,0.40]$ & $0.00$ & $0.90$ \\
SmoothGrad~\cite{smilkov2017smoothgrad}           & gradient & $0.27$ $[0.12,0.40]$ & $0.00$ & $0.90$ \\
Vanilla saliency~\cite{simonyan2014deep}          & gradient & $0.27$ $[0.14,0.40]$ & $0.00$ & $0.90$ \\
Expected Gradients~\cite{erion2021improving}      & gradient & $0.03$ $[0.00,0.10]$ & $0.00$ & $0.90$ \\
\midrule
\multicolumn{2}{@{}l}{Gradient/corr.\ mean}  & $0.29$ $[0.20,0.38]$ & $0.07$ & --- \\
\multicolumn{2}{@{}l}{Causal/interv.\ mean}  & $0.68$ $[0.59,0.78]$ & $0.41$ & --- \\
\botrule
\end{tabular}
\end{table}
