...
faithfulness as a well-chosen single-baseline IG at $K\times$ the compute (for $K$ baseline draws), and it still
...
($0.552$) each draw masked coalitions (random held-out subsets of candidate causes) and average over the genuine responses, so each
...
LIME's surrogate $R^2$, the fit of its local linear model).
...
counterfactual has no effect (a no-op) on some games, where no on-distribution content byte varies
...
column, \emph{interv.}\ and \emph{black-box} (perturb-and-re-run) both belong to the
causal/intervention family, and \emph{gradient} to the gradient/correlational family. The on-distribution
