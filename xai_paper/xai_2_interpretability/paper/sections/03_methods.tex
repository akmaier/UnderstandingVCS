...
\begin{figure}[t]
  ...
  \texttt{jutari} (Julia) $\cdot$ \texttt{jaxtari} (JAX), realizes the three execution modes
  --- HARD, SOFT-STE, and SOFT --- that give four capabilities no neural network grants for
  free: exact forward, exact interventions $\mathrm{do}(u)$, full state
  ...
  by bit-exact re-run, with three companion gradient constructions --- SOFT-STE content-path,
  SOFT fully relaxed, and bilinear sampler --- available depending on the execution mode. Any
  explanation $\hat{E}$ is scored by the correctness triad (right),
...
where the naive gradient vanishes by Prop.~\ref{prop:zero} and only the relaxed soft
gradient, the bilinear-sampler surrogate, or the intervention oracle applies. An \emph{event} output is a discrete state
...
gradient and correlational methods, a gap of $0.34$. Soft-mode gradients, made available through
the temperature-controlled relaxation of the discrete index boundary, restore non-zero signal on
position outputs and narrow the gap. The per-method extreme places an exact causal method,
activation patching, at faithfulness $1.000$ and the field's default saliency at $0.000$ on all
six games. The popular method nonetheless carries the higher plausibility proxy ($0.9$ versus
$0.5$). Against these numbers the semantic gap is measured.
