...
trajectory on one of the six core games (\texttt{breakout}, \texttt{ms\_pacman}, \texttt{pong},
\texttt{qbert}, \texttt{seaquest}, \texttt{space\_invaders}). The candidate-cause set is the
machine's own state. This set is the 128 RAM bytes together with the CPU registers and the TIA
...
faithfulness is the F axis of \Sectionname3.4. The whole-state recording (A8) is included as a
...
We score that output by Pearson correlation against the oracle attribution. The gradient-based
members also inherit the gradient-vanishing failure of the discrete position regime
(Prop.~\ref{prop:zero}). The budgets that matter are the Monte-Carlo or perturbation counts of
the sampling estimators, recorded per method in Table~\ref{supp:tab:phaseB}. The
...
substrate's known circuit or the exact recovered-minus-exact deviation (\Sectionname3.2), and not
...
$\mathrm{do}(u)$ ground truth of \Sectionname3.2, evaluated by re-running the bit-exact machine. It
