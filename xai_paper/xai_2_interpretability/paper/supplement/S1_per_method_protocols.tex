...
state on each game of the scored battery. We state the contract on the six core games
...
to the 42-game scored battery of \Sectionname\ref{supp:scoredbattery}. That state is not the boot screen.
...
available. Its budget is the scored battery (\Sectionname\ref{supp:scoredbattery}) unless noted,
...
budget, the scoring metric, and the committed record. All eight run on the scored battery
(\Sectionname\ref{supp:scoredbattery}), with the six core games as the running example;
...
eight run on the 42-game scored battery (\Sectionname\ref{supp:scoredbattery}), with the six core
...
(\Sectionname\ref{sec:oracle}). The reference is shared and the budget is the scored battery of
\Sectionname\ref{supp:scoredbattery} (six core games as the running example). The budget column records the
...
on the 42-game scored battery (\Sectionname\ref{supp:scoredbattery}), six core games as the running
...
on the 42-game scored battery (\Sectionname\ref{supp:scoredbattery}), with the six core games as the
running example.
\end{table}
