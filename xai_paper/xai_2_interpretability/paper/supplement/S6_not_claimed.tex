...
main-text map (Fig.~5): each VCS failure mode is placed against its documented
...
\S3.2, and the underlying per-method values trace to
...
(Fig.~6): the families of interpretability failure observed on the machine, each annotated
with its faithfulness against the intervention oracle (\S3.2), reported as $F$ with its
...
the discrete position regime, where the naive gradient is provably zero (\S5).}
