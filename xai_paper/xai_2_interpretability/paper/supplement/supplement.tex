...
per-method protocols (Sec.~\ref{supp:protocols}) and the leaderboard schema and its glossary
(Sec.~\ref{supp:schema}). We report the coverage and applicability of every method across games and
output regimes (Sec.~\ref{supp:coverage}) and the mapping from each headline claim to its
evidence (Sec.~\ref{supp:claims}). We trace the provenance of every headline number to the
record file and command that produced it (Sec.~\ref{supp:provenance}). We include a plain
statement of what the paper does not claim (Sec.~\ref{supp:notclaimed}) and the
plausibility-proxy rubric together with its sensitivity analysis
(Sec.~\ref{supp:plausibility}). The two
full-resolution diagnostic figures whose simplified forms appear in the main text are
reproduced here at full density (Figs.~\ref{supp:fig:repmap} and~\ref{supp:fig:taxonomy}).
The correctness triad $\mathrm{F}\wedge\mathrm{S}\wedge\mathrm{M}$ used throughout is the
one defined in the main Methods (\Sectionname3.4); we reference it here rather than restating it. The
benchmark scores 30 interpretability methods together with the intervention oracle (\Sectionname3.2)
