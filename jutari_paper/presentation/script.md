...
`## Slide N` (N is the timeline order, not the beamer page). Segments 5, 6, and 8
are gameplay/animation clips and are written as a guided walkthrough of what is on
screen; their narration plays continuously over the moving footage.

**TTS authoring rules (Chatterbox).** No LaTeX is read out: every symbol is spelled
in English. Sentences stay under roughly twenty words. No em-dashes, semicolons, or
colons in the spoken text (they trigger stutter). `tts_chatterbox.py` synthesises one
sentence at a time, so clean periods give clean audio.
...
## Slide 8 — The relaxation, animated (25 s)
...
## Slide 10 — Gradients with a ground truth (30 s)
...
## Slide 11 — In the supplement (25 s)
