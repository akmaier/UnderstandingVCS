...
Superchip RAM as a cheap random-number source, and \textsc{xitari} seeds that
RAM from \texttt{time(NULL)} while ignoring its own seed parameter, so the
title renders differently on every run despite a pinned seed. This divergence
is invisible to RAM-state checks and visible only on screen. We reach full
...
reference validates the port, and the port can in turn audit the reference for
...
driven by Fable~5; and an early attempt with Kimi-K2.6 (a one-trillion-parameter
...
\begin{proof}
Write the full state as $x=(\text{registers},\text{RAM},\text{RIOT},\text{TIA},
\text{frame buffer})$, and the one-step map $\Phi$ as the per-opcode handler
selected by the byte at the program counter. It is enough to show that, for
each opcode, the soft handler and the hard handler produce the same output
value; the state is then carried forward identically. Every handler is composed
of four kinds of primitive.

\emph{(i) Memory reads.} A ROM or RAM read at integer address $a$ is the one-hot
dot product $\mathrm{peek}(r,a)=\mathbf{1}_a^{\top}r=\sum_i [\![i=a]\!]\,r_i$
\eqref{eq:peek}. Because $a$ is an integer index, $\mathbf{1}_a$ is exact and
$\mathrm{peek}(r,a)=r_a$, identical to a hard array read.

\emph{(ii) Arithmetic.} Register, status-flag, binary-coded-decimal (BCD), and
RIOT-timer updates are evaluated with the same integer and $\mathrm{round}$
operations as the hard handlers; the soft path stores these quantities as
float32 but computes them with integer semantics, so their outputs are equal.

\emph{(iii) Dispatch.} The handler is chosen by a hard switch on the decoded
opcode byte, exactly as in hard mode; the softmax form~\eqref{eq:select} is not
used on the forward path.

\emph{(iv) Branch.} A conditional branch forms the relaxed counter
$\mathrm{pc}_{\mathrm{soft}}$ from the gate~\eqref{eq:branch}, but returns the
...
As $\sg$ is the identity on the forward pass, this equals
...
\begin{proof}
We bound the deviation of the fully relaxed one-step map (softmax
dispatch~\eqref{eq:select} and sigmoid branch~\eqref{eq:branch}, with no
straight-through correction) from the hard map, then propagate it along a
trajectory. Throughout, $C$ bounds the range of any state value (bytes
$\le 255$, branch offsets $\le 256$).

\emph{Dispatch term.} At a step with logits $\ell$ and winner $k^\star$, let the
gap be $\Delta=\ell_{k^\star}-\max_{k\ne k^\star}\ell_k\ge\Delta_{\min}>0$
(Assumption~\ref{ass:dm}). For any loser $k\ne k^\star$,
\[
...
$\sum_k w_k V_k$ of the candidate rows $V_k$,
\[
\begin{aligned}
...
C[(K-1)e^{-\Delta_{\min}/T}+e^{-\alpha m_{\min}}]$, which is~\eqref{eq:bound}.
