...
\begin{itemize}
  ...
  and a differentiable soft path. The Julia port (\textsc{jutari}) is
  bit-for-bit identical to \textsc{xitari} on all 64 supported games---64/64
  byte-identical RAM and 64/64 pixel-identical screens. The JAX port
  (\textsc{jaxtari}) runs and is being brought to the same standard by the
  submission deadline.
  ...
  hard forward pass at any finite temperature (Theorem~\ref{thm:exact}), while
  gradients flow where bit logic provides none; we give the temperature-limit
  error bound for the fully relaxed variant (Theorem~\ref{thm:limit}).
...
object---complex, fully known, and differentiable---and validate it; checking
XAI methods against it is left to future work.
...
disagreement is unambiguously a port bug.
...
hard trajectory rather than an approximation, and a temperature sweep of the
forward error is vacuous---the error is identically zero. The remaining
question is the behaviour of a \emph{fully} relaxed variant that does not apply
the straight-through correction. For that we need a mild non-degeneracy
assumption.
...
sigmoid error by $e^{-\alpha m_{\min}}$, then propagate over $H$ steps with the
...
Register, flag, BCD and RIOT-timer updates use the same integer/$\mathrm{round}$
...
over the $H$ steps, $x^{\mathrm{S}}_t=x^{\mathrm{H}}_t$ for all $t\le H$. Every
...
$e_H\le\delta\,(L^H-1)/(L-1)$. As $T\to0$ and $\alpha\to\infty$,
