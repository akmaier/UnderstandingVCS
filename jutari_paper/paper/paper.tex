...
\title{Can an XAI Method Understand a Game Console?\\
A Differentiable Atari VCS as Ground Truth for Explainable AI}
...
\begin{abstract}
Explainable AI (XAI) is built to reveal why a model decides as it does,
yet it is almost always applied to black boxes for which the true
computation is unknown---so an explanation can be plausible, confident,
and wrong, with no way to tell. This mirrors a famous result in
neuroscience: standard analysis methods, applied to a microprocessor
whose mechanism is fully known, failed to recover how it works. We turn
that provocation into a constructive program. We build two independent,
end-to-end \emph{differentiable} ports of the Atari 2600 Video Computer
System (VCS)---the platform on which deep reinforcement learning was
born---one in Julia (\textsc{jutari}) and one in JAX (\textsc{jaxtari}),
each validated bit-for-bit against the \textsc{xitari} reference. The
Julia port reproduces \textsc{xitari} exactly on all 64 supported
Arcade-Learning-Environment games: 64/64 byte-identical RAM and 64/64
pixel-identical screens. We treat the cartridge ROM as a weight tensor,
the RAM as a soft tape, and control flow as gates, and we prove that the
soft (differentiable) execution is bit-exact equal to the hard execution
in the forward pass at any finite temperature, with gradients flowing
where hard logic has none. The entire system was built in roughly 137
hours of active work, compressed into 29 calendar days, with large parts
written autonomously by coding agents. Driving a bit-exact
re-implementation to pixel exactness even surfaced a latent
non-determinism bug in the reference itself. This is, to our knowledge,
the first XAI study object that is at once a real, complex
information-processing system, fully differentiable, and accompanied by
complete mechanistic ground truth.
\end{abstract}

\section{Introduction}

When we look inside a deep neural network and ask \emph{why} it produced a
particular output, we are doing explainable AI (XAI). Over the past decade
the field has produced a rich toolkit---saliency maps, gradient-based
attribution, concept probes, counterfactuals---and these tools have become
standard instruments for inspecting models we otherwise cannot read. There
is, however, a quiet problem at the heart of the enterprise. Almost every
XAI method is applied to a \emph{black box}: a trained network whose true
internal computation is unknown to us. When the method then reports that
``these pixels mattered,'' we have no independent answer key against which
to check whether the explanation is correct. We can measure whether an
explanation is plausible, or whether humans find it convincing, but not
whether it is \emph{true}.

This situation has a striking precedent outside machine learning. In a
now-famous study, \citet{jonas2017could} asked whether the standard
toolkit of systems neuroscience---connectomics, tuning curves, lesioning,
dimensionality reduction---could recover how a MOS\,6502 microprocessor
computes, using the chip as a model organism precisely because its
mechanism is \emph{completely known}. With perfect and unlimited data,
the methods produced structure that looked meaningful but did not recover
the processor's actual organisation. Their conclusion was not that the
chip is mysterious, but that the \emph{methods} were lacking---and that the
way to find out is to test analysis methods on a system whose ground truth
we already possess. The MOS\,6502 is no accident here: it is the CPU at
the heart of the Atari 2600 Video Computer System (VCS), the very platform
on which modern deep reinforcement learning was demonstrated
\citep{mnih2013playing,mnih2015human,bellemare2013arcade}.

We take this provocation literally, and we invert the usual XAI
experiment. Instead of explaining a black-box agent whose computation is
unknowable, we make the \emph{Atari VCS simulator itself} the subject of
explanation. The simulator is an excellent subject because we know its
true hierarchy bit-for-bit: a 6507 CPU fetches and executes instructions
from a cartridge ROM, a Television Interface Adapter (TIA) turns register
writes into pixels cycle by cycle, and a RIOT chip provides RAM, a timer,
and the joystick and switch I/O. To turn the modern XAI toolkit---which
speaks the language of gradients---onto this hand-written C\texttt{++}
emulator, we make it \emph{differentiable}. We build two independent
ports: \textsc{jutari} in Julia \citep{bezanson2017julia} and
\textsc{jaxtari} in JAX \citep{bradbury2018jax}. Each runs in a
bit-exact \emph{hard} mode and a differentiable \emph{soft} mode, and each
is validated against \textsc{xitari} \citep{xitari}, the Stella-derived
C\texttt{++} emulator \citep{stella} that served as the reference for the
original deep-Q-network (DQN) work.

This paper reports the construction and validation of that substrate, and
a first look at the gradients it exposes. Our contributions are:
\begin{itemize}
  ...
  \item \textbf{A first gradient analysis} on the differentiable substrate,
  establishing that exact ROM-byte and register gradients can be computed
  and localised against known ground truth.
\end{itemize}

We are careful throughout to separate what we have \emph{proven}, what we
have \emph{measured}, and what we \emph{conjecture}. The differentiable
substrate is the instrument; turning the full XAI toolkit on it, and
scoring each method against the known mechanism, is the program this paper
opens rather than closes.

\section{Background and Related Work}

\subsection{XAI Has No Answer Key}
...
\subsection{A First Gradient Analysis}

Because soft mode is forward-exact (Theorem~\ref{thm:exact}), it yields exact
gradients of any scalar read-out with respect to the state and the ROM. As a
sanity check against known ground truth, we differentiate individual rendered
pixels with respect to the TIA colour registers: the gradient of a player-sprite
pixel localises to the player-colour register \texttt{COLUP0}, a background
pixel to \texttt{COLUBK}, and a wall pixel to the playfield-colour register
\texttt{COLUPF}---each pixel attributing to the exactly correct register, which
is the kind of statement that can be \emph{checked} here and cannot be checked
on a black box. Integrated gradients \citep{sundararajan2017axiomatic} over ROM
bytes is the natural next step; our current harness computes it on
short traces, and a full game-length ROM-attribution study is in progress
(some long traces still hit an initialisation busy-loop in the soft RIOT timer,
which we flag as a known limitation rather than a solved result). We present
the register-level localisation as a \emph{proof of concept} that the
substrate exposes correct, ground-truth-checkable gradients; systematic XAI
evaluation is the program this paper enables.
...
that is a genuine, complex information-processing system, fully
differentiable, and accompanied by complete mechanistic ground truth. We built
it twice, in Julia and JAX, validated one port bit-for-bit against the
reference on all 64 supported Atari games, and proved that the differentiable
forward pass is bit-exact equal to the original machine while exposing
gradients where bit logic has none. Where \citet{jonas2017could} asked whether
the methods of one field could understand a microprocessor and found they
could not, we have turned the question into a measurable program for XAI: with
the mechanism fully known and differentiable, every attribution a method
produces can now be scored as right or wrong. The instrument is built; turning
it on the full XAI toolkit, game by game and method by method, is the work
this paper begins.
