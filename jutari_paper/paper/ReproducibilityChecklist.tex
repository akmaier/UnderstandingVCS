...
yes --- we prove the soft (differentiable) forward execution equals the hard execution bit-for-bit at any finite temperature
	...
	partial --- the forward bit-exactness is demonstrated empirically (64/64 byte-identical RAM and pixel-identical screens); the surrogate gradients are shown only qualitatively
...
\begin{itemize}
	...
	partial --- there is no learned-model hyperparameter search; the ranges we do report (batch size 1 to 65536, soft temperature) are stated, without a tuning/selection criterion

	\question{Any code required for pre-processing data is included in the appendix}{(yes/partial/no)}
	yes --- the pre-processing and harness code is included
	...
	yes --- released publicly on acceptance under a research-use licence
	...
	partial --- much of the new code is commented with references to the paper and to xitari, but not uniformly
	...
	yes --- the supplementary \emph{Throughput} section specifies the CPU (Apple~M1~Max) and GPU (NVIDIA GTX~1080~Ti, 11\,GB, and Quadro RTX~5000, 16\,GB, on the FAU LME Slurm cluster) models and memory, plus the software stack and versions (JAX~0.10.1 with CUDA~12, Python~3.12, Julia~1.12, Slurm~21.08.5).
	...
	partial --- throughput is the median of three runs; the conformance results are deterministic (one run, byte-exact, zero variance)
	...
	partial --- throughput is summarised by the median and conformance is exact (zero variance); confidence intervals are not reported
	...
	partial --- the soft temperature, batch sizes, and the 60-NOOP+4-RESET boot are stated, though not consolidated into a single table
