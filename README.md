...
**~1950 tests green** across the two ports (jaxtari ~780 + jutari ~1170). **PXC2** (jaxtari ≡ jutari cross-check) covers 6 ROMs — pong, breakout, space invaders, pitfall, seaquest, enduro — with byte-identical RAM between the two ports at every frame. **PXC1** (jaxtari↔xitari RAM, `*_noop_10` last frame): **pong, breakout, space_invaders, pitfall, enduro bit-exact (0 bytes)** (pitfall via task #81's `getStartingActions` fix; enduro NEWLY bit-exact via task #84's GRP* defer fix — was 8 b/f), seaquest 4. **PXC-S** (per-frame *screen* diff vs xitari): **pong, breakout BIT-EXACT (0 px)** (pong went 32 → 24 → 8 → **0** across tasks #84 GRP* defer, #85 RESMP* gate, and #83 Y_START framebuffer-write gate), space_invaders 42, pitfall 553, seaquest 1043, enduro 774 (post-#84 baseline — the GRP defer exposed unrelated render-timing mismatches in these four; fixtures regenerated as part of #85/#83). See [bug_fix_log.md](bug_fix_log.md) for the scoreboards and the running history of fixes.
