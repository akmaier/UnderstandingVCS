...
**~1950 tests green** across the two ports (jaxtari ~780 + jutari ~1170). **PXC2** (jaxtari ≡ jutari cross-check) covers 6 ROMs — pong, breakout, space invaders, pitfall, seaquest, enduro — with byte-identical RAM between the two ports at every frame. **PXC1** (jaxtari↔xitari RAM, `*_noop_10` last frame): **pong, breakout, space_invaders, pitfall, enduro bit-exact (0 bytes)** (pitfall via task #81's `getStartingActions` fix; enduro NEWLY bit-exact via task #84's GRP* defer fix — was 8 b/f), seaquest 4. **PXC-S** (per-frame *screen* diff vs xitari): **pong, breakout, space_invaders, pitfall ALL BIT-EXACT (0 px)** — pitfall closed by task #94's activation-time VDELP shadow latch (the 6-digit-kernel "time renders in wrong format" bug: GRP0/GRP1 are rewritten 4× per HUD scanline and each digit value lives in the shadow for only ~2 copy slots); space_invaders closed by the same mechanism; seaquest 904 and enduro 516 are at their best-ever levels. See [bug_fix_log.md](bug_fix_log.md) for the scoreboards and the running history of fixes.
