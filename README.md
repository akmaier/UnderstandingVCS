...
**~1950 tests green** across the two ports (jaxtari ~780 + jutari ~1170). **PXC2** (jaxtari ≡ jutari cross-check) covers 6 ROMs — pong, breakout, space invaders, pitfall, seaquest, enduro — with byte-identical RAM between the two ports at every frame. **PXC1** (jaxtari↔xitari RAM, `*_noop_10` last frame): **pong, breakout, space_invaders, pitfall, enduro bit-exact (0 bytes)** (pitfall via task #81's `getStartingActions` fix; enduro NEWLY bit-exact via task #84's GRP* defer fix — was 8 b/f), seaquest 4. **PXC-S** (per-frame *screen* diff vs xitari): **pong, breakout, space_invaders, pitfall ALL BIT-EXACT (0 px)** — pitfall closed by task #94's activation-time VDELP shadow latch (the 6-digit-kernel "time renders in wrong format" bug: GRP0/GRP1 are rewritten 4× per HUD scanline and each digit value lives in the shadow for only ~2 copy slots); space_invaders closed by the same mechanism; seaquest 904 and enduro **249** (task #95: the screen tooling was rendering pitfall/enduro with `GenericRomSettings`, omitting their `getStartingActions` — fixing the settings maps dropped enduro 516→249 and, under random-action gameplay, made **pitfall BIT-EXACT across 600 frames including Harry's jump**; the emulator was correct all along, it was a video/fixture-tooling settings bug). seaquest's 904 is a genuine render divergence — NOT a settings mismatch (seaquest has no `getStartingActions`, so its screen is identical under generic vs seaquest settings; verified — task #96). See [bug_fix_log.md](bug_fix_log.md) for the scoreboards and the running history of fixes.
