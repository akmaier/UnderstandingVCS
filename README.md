...
**~1950 tests green** across the two ports (jaxtari ~780 + jutari ~1170). **PXC2** (jaxtari ≡ jutari cross-check) covers 6 ROMs — pong, breakout, space invaders, pitfall, seaquest, enduro — with byte-identical RAM between the two ports at every frame. **PXC1** (jaxtari↔xitari RAM, `*_noop_10` last frame): **pong, breakout, space_invaders, pitfall, enduro bit-exact (0 bytes)** (pitfall via task #81's `getStartingActions` fix; enduro NEWLY bit-exact via task #84's GRP* defer fix — was 8 b/f), seaquest 4. **PXC-S** (per-frame *screen* diff vs xitari): breakout **0 px (BIT-EXACT)**, **pong 8** (was 32 — task #84 GRP* defer killed the paddle 1-row-early bug; task #85 RESMP* gate killed the 16 px rows 35-37 score-sprite phantoms; remaining 8 = row-0 HMOVE comb [#83]), space_invaders 42, pitfall 553, seaquest 1043, enduro 774 (latter four are the post-#84 true baseline after the GRP defer exposed unrelated render-timing mismatches; fixtures regenerated as part of #85). See [bug_fix_log.md](bug_fix_log.md) for the scoreboards and the running history of fixes.
