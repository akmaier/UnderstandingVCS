...
**~1950 tests green** across the two ports (jaxtari ~780 + jutari ~1170). **PXC2** (jaxtari ≡ jutari cross-check) covers 6 ROMs — pong, breakout, space invaders, pitfall, seaquest, enduro — with byte-identical RAM between the two ports at every frame. **PXC1** (jaxtari↔xitari RAM, `*_noop_10` last frame): **pong, breakout, space_invaders, pitfall bit-exact (0 bytes)** (pitfall newly added via task #81's `getStartingActions` fix — was 19), seaquest 4, enduro **partial** (was 43; task #82 starting-actions closes the bulk, residual ≈ 3 b/f from a separate boot-state mismatch). **PXC-S** (a new test the P3i-g pt4 work added — per-frame *screen* diff vs xitari): breakout **0 px (BIT-EXACT)**, pong **32**, space_invaders **12** (near bit-exact), pitfall 322, seaquest 1104, enduro 1197 (down from 29760/2145/1786/3941/1972 respectively across pt5–pt8 — pt8's RIOT INTIM `-1` fix dropped the total by ~74% in one shot; see [bug_fix_log.md](bug_fix_log.md)). See [bug_fix_log.md](bug_fix_log.md) for the scoreboards and the running history of fixes.
