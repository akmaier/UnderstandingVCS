...
**~1950 tests green** across the two ports (jaxtari ~780 + jutari ~1170). **PXC2** (jaxtari ≡ jutari cross-check) covers 6 ROMs — pong, breakout, space invaders, pitfall, seaquest, enduro — with byte-identical RAM between the two ports at every frame. **PXC1** (jaxtari↔xitari RAM, `*_noop_10` last frame): **pong, breakout, space_invaders bit-exact (0 bytes)**, seaquest 4, pitfall 19, enduro 43 (= the pre-P3i-g parent baseline; the original "29" pin was stale). **PXC-S** (a new test the P3i-g pt4 work added — per-frame *screen* diff vs xitari): breakout **8 px**, pong **920 px**, others 1700–3940 px (pre-existing rendering-accuracy gaps now pinned and chased one by one). See [bug_fix_log.md](bug_fix_log.md) for the scoreboards and the running history of fixes.
