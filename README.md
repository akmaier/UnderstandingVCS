...
**~1950 tests green** across the two ports (jaxtari ~780 + jutari ~1170). **PXC2** (the strong jaxtari ≡ jutari cross-check) covers 6 ROMs — pong, breakout, space invaders, pitfall, seaquest, enduro — with byte-identical RAM between the two ports at every frame. **PXC1** (the bit-exact jaxtari ↔ xitari claim) is still xfail on pong (10-byte residual gap) but the root cause is now well-characterized as `data_bus_state` drift from missing 6502 internal-cycle bus exposures; per-ROM divergence ranges from 0 (space invaders) to 47 bytes (enduro), and joystick-vs-paddle is *not* the deciding factor.
