...
# --- P1h functional handlers (Zygote-friendly mirror) --------------------- #
#
# Same 27 NOPs + 6 LAX + 4 SAX = 37 opcodes as the mutating P1h block,
# but built from the functional `_func_bus_read` / `_func_bus_write` /
# `update_state` primitives so Zygote can differentiate through them.

function _func_nop_implied_1byte(state, bus)
    return update_state(state; PC=state.PC + 1f0, cycles=state.cycles + 2f0), bus
end

function _func_nop_imm(state, bus)
    _ = _operand_byte(bus, state.PC + 1f0)
    return update_state(state; PC=state.PC + 2f0, cycles=state.cycles + 2f0), bus
end

function _func_nop_zp(state, bus)
    _ = _func_bus_read(bus, _addr_zp(state, bus))
    return update_state(state; PC=state.PC + 2f0, cycles=state.cycles + 3f0), bus
end

function _func_nop_zp_x(state, bus)
    _ = _func_bus_read(bus, _addr_zp_x(state, bus))
    return update_state(state; PC=state.PC + 2f0, cycles=state.cycles + 4f0), bus
end

function _func_nop_abs(state, bus)
    _ = _func_bus_read(bus, _addr_abs(state, bus))
    return update_state(state; PC=state.PC + 3f0, cycles=state.cycles + 4f0), bus
end

function _func_nop_abs_x(state, bus)
    _ = _func_bus_read(bus, _addr_abs_x(state, bus))
    return update_state(state; PC=state.PC + 3f0, cycles=state.cycles + 4f0), bus
end

function _func_do_lax(state, bus, value::Real, instr_len::Real, cycles::Real)
    new_p = _set_nz(state.P, value)
    return _with_p(state, new_p;
        A=Float32(value), X=Float32(value),
        PC=state.PC + Float32(instr_len),
        cycles=state.cycles + Float32(cycles),
    ), bus
end

_func_lax_zp(s, b)    = _func_do_lax(s, b, _func_bus_read(b, _addr_zp(s, b)),    2, 3)
_func_lax_zp_y(s, b)  = _func_do_lax(s, b, _func_bus_read(b, _addr_zp_y(s, b)),  2, 4)
_func_lax_abs(s, b)   = _func_do_lax(s, b, _func_bus_read(b, _addr_abs(s, b)),   3, 4)
_func_lax_abs_y(s, b) = _func_do_lax(s, b, _func_bus_read(b, _addr_abs_y(s, b)), 3, 4)
_func_lax_ind_x(s, b) = _func_do_lax(s, b, _func_bus_read(b, _addr_ind_x(s, b)), 2, 6)
_func_lax_ind_y(s, b) = _func_do_lax(s, b, _func_bus_read(b, _addr_ind_y(s, b)), 2, 5)

function _func_do_sax(state, bus, addr::Real, instr_len::Real, cycles::Real)
    value   = Float32(Int(state.A) & Int(state.X))
    new_bus = _func_bus_write(bus, addr, value)
    return update_state(state;
        PC=state.PC + Float32(instr_len),
...
_func_sax_zp(s, b)    = _func_do_sax(s, b, _addr_zp(s, b),    2, 3)
_func_sax_zp_y(s, b)  = _func_do_sax(s, b, _addr_zp_y(s, b),  2, 4)
_func_sax_abs(s, b)   = _func_do_sax(s, b, _addr_abs(s, b),   3, 4)
_func_sax_ind_x(s, b) = _func_do_sax(s, b, _addr_ind_x(s, b), 2, 6)
...
const _FUNC_HANDLERS = let h = Function[_func_default for _ in 1:256]
    ...
    # RTI
    h[0x40 + 1] = _func_rti
    # P1h functional handlers — 27 NOP variants + 6 LAX + 4 SAX.
    # Implied 1-byte NOPs.
    h[0x1A + 1] = _func_nop_implied_1byte
    h[0x3A + 1] = _func_nop_implied_1byte
    h[0x5A + 1] = _func_nop_implied_1byte
    h[0x7A + 1] = _func_nop_implied_1byte
    h[0xDA + 1] = _func_nop_implied_1byte
    h[0xFA + 1] = _func_nop_implied_1byte
    # Immediate 2-byte NOPs.
    h[0x80 + 1] = _func_nop_imm
    h[0x82 + 1] = _func_nop_imm
    h[0x89 + 1] = _func_nop_imm
    h[0xC2 + 1] = _func_nop_imm
    h[0xE2 + 1] = _func_nop_imm
    # Zero-page NOPs.
    h[0x04 + 1] = _func_nop_zp
    h[0x44 + 1] = _func_nop_zp
    h[0x64 + 1] = _func_nop_zp
    # Zero-page,X NOPs.
    h[0x14 + 1] = _func_nop_zp_x
    h[0x34 + 1] = _func_nop_zp_x
    h[0x54 + 1] = _func_nop_zp_x
    h[0x74 + 1] = _func_nop_zp_x
    h[0xD4 + 1] = _func_nop_zp_x
    h[0xF4 + 1] = _func_nop_zp_x
    # Absolute NOP.
    h[0x0C + 1] = _func_nop_abs
    # Absolute,X NOPs.
    h[0x1C + 1] = _func_nop_abs_x
    h[0x3C + 1] = _func_nop_abs_x
    h[0x5C + 1] = _func_nop_abs_x
    h[0x7C + 1] = _func_nop_abs_x
    h[0xDC + 1] = _func_nop_abs_x
    h[0xFC + 1] = _func_nop_abs_x
    # LAX (6 modes).
    h[0xA7 + 1] = _func_lax_zp
    h[0xB7 + 1] = _func_lax_zp_y
    h[0xAF + 1] = _func_lax_abs
    h[0xBF + 1] = _func_lax_abs_y
    h[0xA3 + 1] = _func_lax_ind_x
    h[0xB3 + 1] = _func_lax_ind_y
    # SAX (4 modes).
    h[0x87 + 1] = _func_sax_zp
    h[0x97 + 1] = _func_sax_zp_y
    h[0x8F + 1] = _func_sax_abs
    h[0x83 + 1] = _func_sax_ind_x
    h
end
